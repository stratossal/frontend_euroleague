...
export function Pagination({ players, itemsPerPage = 5 }: PaginationProps) {
    ...
    const columns: {
        ...
        { label: "Player", key: "name", sortable: false, width: "w-[200px]", align: "left" },
        { label: "Position", key: "position", sortable: false, width: "w-[100px]", align: "left" },
        { label: "Team", key: "team", sortable: false, width: "w-[150px]", align: "left" },
        { label: "Age", key: "age", sortable: true, width: "w-[60px]", align: "right", defaultSort: "desc" },
        { label: "PPG", key: "stats.pointsPerGame", sortable: true, width: "w-[60px]", align: "right", defaultSort: "desc" },
        { label: "RPG", key: "stats.reboundsPerGame", sortable: true, width: "w-[60px]", align: "right", defaultSort: "desc" },
        { label: "APG", key: "stats.assistsPerGame", sortable: true, width: "w-[60px]", align: "right", defaultSort: "desc" },
    ];
    ...
    const sortedPlayers = [...players].sort((a, b) => {
        ...
        const aValue = sortConfig.key
            .split(".")
            .reduce((obj: any, key) => obj?.[key], a);

        const bValue = sortConfig.key
            .split(".")
            .reduce((obj: any, key) => obj?.[key], b);
    ...
    const requestSort = (key: string) => {
        ...
        let direction: "asc" | "desc" = column?.defaultSort || "asc";
                    ...
                    <TableRow>
                        {columns.map((col) =>
                            col.sortable ? (
                                ...
                                <TableHead
                                    key={col.key}
                                    className={`${col.width} px-4 ${col.align === "right" ? "text-right" : "text-left"}`}
                                >
                ...
                <TableBody>
                    {currentItems.map((player) => (
                        ...
                        >
                            ...
                            <TableCell className="text-right w-[60px] px-4 font-semibold">{player.stats.pointsPerGame}</TableCell>
                            <TableCell className="text-right w-[60px] px-4">{player.stats.reboundsPerGame}</TableCell>
                            <TableCell className="text-right w-[60px] px-4 font-semibold">{player.stats.assistsPerGame}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
